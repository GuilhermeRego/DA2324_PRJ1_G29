    ...
    while (true) {
        ...
        switch (option) {
            case 1: {
                ...
                cout << "Maximum flow from reservoirs to city " << sink << ": " << min(demand,maxFlow) << endl;

                // Write the maximum flow to a file
                ofstream outputFile("max_flow_reservoir_to_city.txt", ios::app);
                if (outputFile.is_open()) {
                    outputFile << "Maximum flow from reservoirs to city " << sink << ": " << min(maxFlow,demand) << endl;
                    outputFile.close();
            ...
            case 2: {
                ...
                for (auto &city : dataManager.getCities()) {
                    ...
                    cout << "City: " << city.second.getName() << ", Max Flow: " << min(demand,maxFlow) << endl;
                }
